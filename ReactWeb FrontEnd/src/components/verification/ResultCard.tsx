      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
        <div className={cn("p-8 border-b flex flex-col md:flex-row md:items-center justify-between gap-4", getVerdictStyles(normalizedVerdict))}>
          ...
          <div className="flex gap-2">
             ...
             >
               ...
               {showDetails ? "Ẩn chi tiết" : "Chi tiết phân tích"}
             </button>
          </div>
        </div>
        ...
        {showDetails && result.signals && (
          <div className="px-8 pb-8 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-6">
              <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Tiến trình phân tích đa tầng (Waterfall)</h5>
              {result.signals.map((sig) => (
                <div key={sig.gate} className="space-y-3 pb-6 border-b border-slate-200/50 last:border-0 last:pb-0">
                   <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 rounded-full bg-white shadow-sm border border-slate-200 flex items-center justify-center text-xs font-black text-slate-500">
                           {sig.gate}
                         </div>
                         <span className="text-sm font-black text-slate-700 uppercase tracking-tight">{sig.name}</span>
                      </div>
                      <div className="flex items-center gap-4">
                         <span className={cn(
                           "text-[10px] font-black uppercase px-2 py-0.5 rounded-md",
                           getNormalizedVerdict(sig.verdict) === "True" ? "bg-emerald-100 text-emerald-700" :
                           getNormalizedVerdict(sig.verdict) === "Fake" ? "bg-rose-100 text-rose-700" :
                           "bg-amber-100 text-amber-700"
                         )}>
                           {getVerdictLabel(getNormalizedVerdict(sig.verdict))}
                         </span>
                         {sig.confidence && (
                           <div className="flex flex-col items-end">
                              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Độ tin cậy</span>
                   ...
                   {sig.reasoning && (
                     <div className="pl-11 pr-4 py-3 bg-white/60 rounded-xl border border-slate-100">
        ...
        <div className="px-8 pb-8 flex flex-wrap gap-2">
            <div className="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                ...
                Kết luận tại: Gate {result.gate_fired || "?"} ({result.source})
            </div>
        </div>
      </div>
